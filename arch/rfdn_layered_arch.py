...
class RFDN_layered(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
                 conv='DepthWiseConv', upsampler='pixelshuffledirect', p=0.25):  #, reduce_c=6
        super(RFDN_layered, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
        ...
        # self.map1 = nn.Linear(num_feat, num_feat)

        self.B2 = RFDB(in_channels=num_feat, conv=self.conv, p=p)
        self.map2 = nn.Linear(num_feat,100)

        self.B3 = RFDB(in_channels=100, conv=self.conv, p=p)
        # self.map3 = nn.Linear(100, 100)

        self.B4 = RFDB(in_channels=100, conv=self.conv, p=p)
        self.map4 = nn.Linear(100, num_feat)

        self.B5 = RFDB(in_channels=num_feat, conv=self.conv, p=p)
        # self.map5 = nn.Linear(num_feat - 4 * reduce_c, num_feat - 5 * reduce_c)

        self.B6 = RFDB(in_channels=num_feat, conv=self.conv, p=p)
        # self.B7 = RFDB(in_channels=num_feat, conv=self.conv, p=p)
        total = num_feat*4+200
