...
class ESA(nn.Module):
    ...
    def forward(self, input):
        ...
        v_range = self.hswish(self.conv_max(v_max))
        c3 = self.hswish(self.conv3(v_range))
        c3 = self.conv3_(c3)
...
class RFDN_hswish(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
        ...
        super(RFDN_hswish, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
    ...
    def forward(self, input):
        ...
        out_B = self.hswish(out_B.permute(0, 3, 1, 2))
        # print(out_B.shape)
