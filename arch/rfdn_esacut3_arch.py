...
class ESA(nn.Module):
    def __init__(self, num_feat=50, conv=nn.Conv2d, p=0.25):
        ...
        self.maxPooling = nn.MaxPool2d(kernel_size=7, stride=6)
    ...
    def forward(self, input):
        ...
        v_max = self.maxPooling(c1_.permute(0, 3, 1, 2))
        ...
        v_range = F.interpolate(v_range, (input.size(2), input.size(3)), mode='bilinear', align_corners=False)
        cf = self.conv_f(c1_)
        c4 = self.conv4((v_range.permute(0, 2, 3, 1) + cf))
...
class RFDN_esacut3(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
        ...
        super(RFDN_esacut3, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
